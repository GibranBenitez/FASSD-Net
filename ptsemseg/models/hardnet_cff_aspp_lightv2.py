...
class _ASPPModule(nn.Module):
    def __init__(self, inplanes, planes, kernel_size, padding, dilation, BatchNorm):
        ...
        if padding == 0:
            print("Asymetric ASPP module not supported for conv 1x1")
...
class ASPP(nn.Module):
    def __init__(self, inplanes, BatchNorm):
        ...
        # inplanes = 512
        dilations = [1, 12, 24, 36]  # [1, 6, 12, 18]
        mid_planes = int(inplanes/2)
        ...
        self.aspp2 = _ASPPModule(inplanes, mid_planes, 3, padding=dilations[1], dilation=dilations[1], BatchNorm=BatchNorm)
        self.aspp3 = _ASPPModule(inplanes, mid_planes, 3, padding=dilations[2], dilation=dilations[2], BatchNorm=BatchNorm)
        self.aspp4 = _ASPPModule(inplanes, mid_planes, 3, padding=dilations[3], dilation=dilations[3], BatchNorm=BatchNorm)
...
class DABModule(nn.Module):
    ...
    def forward(self, input):
        ...
        pConv = self.parallel_conv3x3(output)
...
class hardnet(nn.Module):
    def __init__(self, n_classes=19):
        ...
        ### Asymetric ASPP
        self.aspp = build_aspp(inplanes = prev_block_channels, BatchNorm=nn.BatchNorm2d)
        ...
        for i in range(n_blocks-1,-1,-1):
            ...
            self.Dab_leo.append(DABModule(cur_channels_count//2, d=dilation_block[i]))
            # print("========== i is: ", i)
            cur_channels_count = cur_channels_count//2  # Error when using DAB 

            # blk = HarDBlock(cur_channels_count, gr[i], grmul, n_layers[i])
            blk = ConvLayer(cur_channels_count, 64)
            
            self.denseBlocksUp.append(blk)
            prev_block_channels = 64# blk.get_out_ch()
    ...
    def forward(self, x):
        ...
        for i in range(self.n_blocks):
            ...
            out = self.Dab_leo[i](out)

            out = self.denseBlocksUp[i](out)
            # out = self.stdConvUp[i](out)
